...
def addLayer(inputs, in_size, out_size, activationFunction=None,layerName='layer'):
    ...
    with tf.name_scope(layerName):          #可视化图层分级
        ...
        tf.summary.histogram(layerName+'_Weights', Weights)
        with tf.name_scope(layerName+'_biases'):
            biases = tf.Variable(tf.zeros([1, out_size]) + 0.1, name='b')
        tf.summary.histogram(layerName+'_biases', biases)
        ...
        tf.summary.histogram(layerName + '_outputs', outputs)
        return outputs
...
def main():
    ...
    # 画神经网络图
    picPrediction = addLayer(picData, 41, 23, activationFunction=tf.nn.softmax, layerName='L1')
    # 计算loss/cost
    ...
    with tf.Session() as sess:
        ...
        for step in range(1001):
            ...
            # print("qwqw"+str(len(label_train_batch[0])))
